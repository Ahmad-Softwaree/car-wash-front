...
const CarModelCard = lazy(() => import("@/components/cards/CarModelCard"));

import {
  useDeleteCarModel,
  useGetDeletedCarModels,
  useGetCarModels,
  useRestoreCarModel,
  useSearchDeletedCarModels,
  useSearchCarModels,
...
const CarModels = () => {
  ...
  const { mutateAsync, isPending } = useDeleteCarModel();
  ...
  return (
    <>
      <Container
        ...
        <Pagination<CarModel[]>
          queryFn={() =>
            deleted_page ? useGetDeletedCarModels() : useGetCarModels()
          }
          searchQueryFn={() =>
            deleted_page
              ? useSearchDeletedCarModels(
              ...
              <div className="w-full max-w-full overflow-x-auto max-h-screen hide-scroll">
                <Table className="relative  w-full table-dark-light !text-primary-800 dark:!text-white  default-border">
                              ...
                              if (checked.length == 0) {
                                dispatch({
                                  ...
                                  payload: allData
                                    .slice(0, 30)
                                    .map(
                                      (val: CarModel, _index: number) => val.id
                                    ),
                                });
                              } else {
                                dispatch({
                  ...
                  <TBody className="w-full ">
                    <>
                      {allData?.map((val: CarModel, index: number) => (
                        <CarModelCard key={val.id} index={index} {...val} />
                      ))}
      ...
      {isAddOpen && (
        <Dialog
          ...
          <CarModelForm state="insert" onClose={() => setIsAddOpen(false)} />
        </Dialog>
      )}
    </>
  );
