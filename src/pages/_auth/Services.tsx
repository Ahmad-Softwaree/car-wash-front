...
const ServiceCard = lazy(() => import("@/components/cards/ServiceCard"));
  ...
  return (
    <>
      <Container
        ...
        <Pagination<Service[]>
          queryFn={() =>
            deleted_page ? useGetDeletedServices() : useGetServices()
          }
          searchQueryFn={() =>
            deleted_page
              ? useSearchDeletedServices(
              ...
              <div className="w-full max-w-full overflow-x-auto max-h-screen hide-scroll">
                <Table className="relative  w-full table-dark-light !text-primary-800 dark:!text-white  default-border">
                              ...
                              if (checked.length == 0) {
                                dispatch({
                                  ...
                                  payload: allData
                                    .slice(0, 30)
                                    .map(
                                      (val: Service, _index: number) => val.id
                                    ),
                                });
                              } else {
                                dispatch({
                  ...
                  <TBody className="w-full ">
                    <>
                      {allData?.map((val: Service, index: number) => (
                        <ServiceCard key={val.id} index={index} {...val} />
                      ))}
      ...
      {isAddOpen && (
        <Dialog
          ...
          <ServiceForm state="insert" onClose={() => setIsAddOpen(false)} />
        </Dialog>
      )}
    </>
  );
