import { CarModelCardProps } from "@/types/car-model";
import { PenLine, Trash } from "lucide-react";
import { useState } from "react";
import Dialog from "../shared/Dialog";
import CarModelForm from "../forms/CarModelForm";
import DeleteModal from "../ui/DeleteModal";
import { useDeleteCarModel } from "@/lib/react-query/query/car-model.query";
import { useGlobalContext } from "@/context/GlobalContext";
import { CONTEXT_TYPEs } from "@/context/types";

const CarModelCard = ({ id, name }: CarModelCardProps) => {
  const [isDelete, setIsDelete] = useState<boolean>(false);
  const [isUpdate, setIsUpdate] = useState<boolean>(false);
  const { mutateAsync, isPending } = useDeleteCarModel(id);
    ...
    <>
      ...
      {isUpdate && (
        <Dialog
          ...
          isOpen={isUpdate}
          onClose={closeUpdate}>
          <CarModelForm state="update" onClose={closeUpdate} />
          <button
