...
export default function DatePicker({
  ...
  selectedStartDate: NullableDate;
  selectedEndDate: NullableDate;
  setSelectedStartDate: React.Dispatch<React.SetStateAction<NullableDate>>;
  setSelectedEndDate: React.Dispatch<React.SetStateAction<NullableDate>>;
}) {
  ...
  const renderCalendar = (): JSX.Element[] => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      if (
        selectedStartDate &&
        dayString === selectedStartDate.toLocaleDateString("en-US")
      ) {
        className += "!light-dark rounded-r-none ";
      }
      if (
        selectedEndDate &&
        dayString === selectedEndDate.toLocaleDateString("en-US")
      ) {
        className += "!light-dark rounded-l-none ";
      }
      if (
        selectedStartDate &&
        selectedEndDate &&
        ...
        className += "light-dark rounded-none";
      }
    ...
    <div className="relative" ref={datepickerRef}>
      <div className="relative flex items-center">
        ...
        <input
          ...
          className="w-full text-sm rounded-lg border border-stroke bg-transparent py-2.5 pl-[50px] pr-8 text-dark-2 outline-none transition focus:border-primary dark:border-dark-3 dark:text-dark-6 dark:focus:border-primary"
          value={updateInput()}
      ...
      {isOpen && (
        <div
          ...
          className="shadow-datepicker relative mt-2 rounded-xl border border-stroke dark-light pt-5 !z-[3000]"
        >
          <div className="flex items-center justify-between px-5">
            <button
              title="prevMonth"
              id="prevMonth"
              className="rounded-md px-2 py-2 hover:light-dark"
              onClick={() =>
                setCurrentDate(
          ...
          <div className="mb-4 mt-6 grid grid-cols-7 gap-2 px-5">
            {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
              <div
                key={day}
                className="text-center text-sm font-medium text-secondary-color hover:light-dark"
              >
                {day}
              </div>
            ))}
          ...
          <div className="mt-5 flex justify-start space-x-2.5 border-t border-stroke p-5 dark:border-dark-3 gap-2">
            <Button
              variant="soft"
              color="danger"
              id="cancelButton"
              className="rounded-lg !font-bukra border border-primary px-5 py-2.5 text-xs  hover:bg-blue-light-5"
