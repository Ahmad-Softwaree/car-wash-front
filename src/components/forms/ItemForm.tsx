...
const ItemForm = ({
  ...
  const [addItemType, setAddItemType] = useState<boolean>(false);
  const { state: globalState } = useGlobalContext();
  const form = useRef<FormHandle>(null);
  const { mutateAsync, isPending } = useAddItem();
  const { mutateAsync: changeQuantity } = useChangeItemQuantity(
    globalState?.oldData?.id
  );
  const [quantityInput, setQuantityInput] = useState<string>("");
  const { mutateAsync: deleteImageFunc, isPending: deleteImageLoading } =
    useDeleteItemImage(globalState?.oldData?.id);
    ...
    <>
      ...
      >
        ...
        >
          ...
          <div className="col-span-full lg:col-span-5 flex flex-col justify-start items-start gap-5 py-5 w-full h-full">
            ...
            {globalState.oldData?.image_url && !imageDeleted && (
      ...
      {addItemType && (
        <Dialog
          ...
          maxHeight={`90%`}
          isOpen={addItemType}
          onClose={() => setAddItemType(false)}
        >
          <CustomClose onClick={() => setAddItemType(false)} />
          <ItemTypeForm state="insert" onClose={() => setAddItemType(false)} />
        </Dialog>
      )}
    </>
  );
